...
class transposh_3rdparty {
	...
	public function super_cache_invalidate() {
		...
		$GLOBALS['wp_cache_request_uri'] = preg_replace( '/[ <>\'\"\r\n\t\(\)]/', '', str_replace( '/index.php', '/',
			str_replace( '..', '', preg_replace( "/(\?.*)?$/", '', $GLOBALS['wp_cache_request_uri'] ) ) ) );
	...
	public function bp_uri_filter( $uri ) {
		...
		if ( $this->transposh->options->enable_url_translate ) {
			$uri = transposh_utils::get_original_url( $uri, '', $lang,
				array( $this->transposh->database, 'fetch_original' ) );
		}
	...
	public function bp_activity_after_save( $params ) {
		...
		if ( transposh_utils::get_language_from_url( $_SERVER['HTTP_REFERER'], $this->transposh->home_url ) ) {
			bp_activity_update_meta( $params->id, 'tp_language',
				transposh_utils::get_language_from_url( $_SERVER['HTTP_REFERER'], $this->transposh->home_url ) );
		}
	}
	...
	public function transposh_buddypress_stream( $translation, $original, $lang ) {
		...
		$values = array(
			...
			'action'            => sprintf( __( '%s translated a phrase to %s with transposh:', TRANSPOSH_TEXT_DOMAIN ),
				bp_core_get_userlink( $bp->loggedin_user->id ), substr( transposh_consts::$languages[ $lang ], 0,
					strpos( transposh_consts::$languages[ $lang ], ',' ) ) ),
	...
	public function fix_wpbdp_cat_links( $url ) {
		...
		return transposh_utils::rewrite_url_lang_param( $url, $this->transposh->home_url,
			$this->transposh->options->enable_permalinks, $this->transposh->target_language,
