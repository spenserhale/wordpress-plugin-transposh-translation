...
class transposh_database {
	...
	public function __construct( &$transposh ) {
		...
		if ( class_exists( 'Memcache' ) ) {
			if ( $this->transposh->options->debug_enable ) {
				tp_logger( 'Trying pecl-Memcache!' );
			}
	...
	public function fetch_translation( string $orig, string $lang ): ?array {
		...
		if ( isset( $this->translations[ $original ] ) ) {
			...
			tp_logger( "prefetch result for $original >>> {$this->translations[$original][0]} ({$this->translations[$original][1]})",
				4 );
		} else {
			// make sure $lang is reasonable, unless someone is messing with us, it will be ok
			if ( ! ( $this->transposh->options->is_active_language( $lang ) ) ) {
	...
	public function fetch_original( string $trans, string $lang ): ?string {
		...
		if ( $this->translations[ $translation ] ) {
			...
			tp_logger( "prefetch result for $translation >>> {$this->translations[$translation][0]} ({$this->translations[$translation][1]})" );
		} else {
...
//                $original = transposh_utils::base64_url_decode($_POST["tk$i"]);
			...
			if ( $translated_text ) {
				if ( $source > 0 ) {
					tp_logger( "Warning auto-translation for already translated: $original $lang, $old_source - $translated_text",
						1 );
					continue;
	...
	public function get_translation_history( $token, $lang ): void {
		...
		// check params
		tp_logger( "Enter " . __FILE__ . " Params:  $token, $lang, $ref" );
		if ( ! isset( $token, $lang ) ) {
	...
	public function db_stats(): void {
		...
		foreach ( $rows as $row ) {
			if ( $row->count ) {
				printf( '<p>' . __( 'Total of <strong style="color:red">%s</strong> translated phrases.',
						TRANSPOSH_TEXT_DOMAIN ) . '</p>', $row->count );
			}
		}
		...
		foreach ( $rows as $row ) {
			if ( $row->count ) {
				printf( '<p>' . __( '<strong>%1s</strong> has <strong style="color:red">%2s</strong> human translated phrases.',
						TRANSPOSH_TEXT_DOMAIN ) . '</p>', $row->lang, $row->count );
			}
		}
		...
		foreach ( $rows as $row ) {
			...
			printf( '<p>' . __( 'On <strong>%1s</strong><br/>user <strong>%2s</strong> translated<br/>"<strong>%3s</strong>"<br/>to <strong style="color:red">%4s</strong><br/>"<strong>%5s</strong>"',
					TRANSPOSH_TEXT_DOMAIN ) . '</p>', $td, $row->translated_by, $row->original, $row->lang,
				$row->translated );
		}
	}
	...
	public function db_maint(): void {
		...
		$removetranslogextras = "DELETE {$this->translation_log_table} FROM {$this->translation_log_table} INNER JOIN {$this->translation_table} ON " .
		                        ...
		                        "{$this->translation_table}.timestamp = {$this->translation_log_table}.timestamp";
		tp_logger( $removetranslogextras );
		...
		tp_logger( $removetranslationsofnothing );
