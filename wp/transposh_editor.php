...
class transposh_editor_table extends WP_List_Table {
	...
	public function column_lang( $item ) {
		...
		return sprintf( '%1$s %2$s', transposh_consts::get_language_name( $item['lang'] ),
	...
	public function column_translated_by( $item ) {
		...
		$actions = array(
			// 'edit' => sprintf('<a href="?page=%s&action=%s&book=%s">Edit</a>', $_REQUEST['page'], 'edit', 1/*$item['ID']*/),
			'filter' => sprintf( '<a href="?page=%s&action=%s&ftb=%s">Filter</a>', $_REQUEST['page'], 'filter-by',
				$item['translated_by'] ),
		);
	...
	public function prepare_items() {
		...
		$orderby               = ( ! empty( $_GET['orderby'] ) ) ? filter_input( INPUT_GET, 'orderby',
			FILTER_SANITIZE_SPECIAL_CHARS ) : 'timestamp';
		$order                 = ( ! empty( $_GET['order'] ) ) ? filter_input( INPUT_GET, 'order',
			FILTER_SANITIZE_SPECIAL_CHARS ) : 'desc';
		...
		global $my_transposh_plugin;
		$this->items = $my_transposh_plugin->database->get_filtered_translations( '0', 'null', "$limit, $per_page",
	...
	public function render_table() {
		...
		if ( $this->filter ) {
			...
			echo( sprintf( "<a href='%s'>%s</a>",
				esc_url( remove_query_arg( array( 'action', 'ftb', 'fl', 'paged' ), $current_url ) ),
				__( 'Remove filter' ) ) );
		}
		echo '
        <form method="post">
