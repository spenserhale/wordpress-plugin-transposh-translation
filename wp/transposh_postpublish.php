	...
	function on_admin_menu() {
		...
		foreach ( $post_types as $post_type ) {
			...
			if ( $this->transposh->options->enable_autoposttranslate ) {
				add_meta_box( 'transposh_postpublish', __( 'Transposh', TRANSPOSH_TEXT_DOMAIN ),
					array( &$this, "transposh_postpublish_box" ), $post_type, 'side', 'core' );
			}
			add_meta_box( 'transposh_setlanguage', __( 'Set post language', TRANSPOSH_TEXT_DOMAIN ),
				array( &$this, "transposh_setlanguage_box" ), $post_type, 'advanced', 'core' );
		}
		if ( ! isset( $_GET['post'] ) ) {
			return;
		}
		if ( get_post_meta( $_GET['post'], 'transposh_can_translate', true ) ) { // do isdefined stuff
			...
			delete_post_meta( $_GET['post'],
				'transposh_can_translate' ); // as we have used the meta - it can go now, another option would have been to put this in the getphrases
		}
	}
