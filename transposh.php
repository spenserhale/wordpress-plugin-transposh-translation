...
//        add_action('save_post', array(&$this, 'transposh_save_post_language'));
	...
	public function on_redirect_canonical( $red, $req ) {
		...
		if ( ! $this->options->is_default_language( $this->target_language ) ) {
			$red = str_replace( array( '%2F', '%3A', '%3B', '%3F', '%3D', '%26' ),
				array( '/', ':', ';', '?', '=', '&' ), urlencode( $this->rewrite_url( $red ) ) );
		}
	...
	public function get_clean_url() {
		...
		if ( $this->options->enable_url_translate ) {
			$this->clean_url = transposh_utils::get_original_url( $this->clean_url, '', $this->target_language,
				array( $this->database, 'fetch_original' ) );
		}
	...
	public function process_page( $buffer ): string { //php7?
		...
		foreach ( headers_list() as $header ) {
			if ( stripos( $header, 'Content-Type:' ) !== false ) {
				tp_logger( $header );
				if ( stripos( $header, 'text' ) === false && stripos( $header, 'json' ) === false && stripos( $header,
						'rss' ) === false ) {
		...
		elseif ( $this->options->is_default_language( $this->target_language ) && ! $this->options->enable_default_translate ) {
			...
			tp_logger( "Translating {$_SERVER['REQUEST_URI']} to: {$this->target_language} for: {$_SERVER['REMOTE_ADDR']}",
				1 );
			...
			$parse->change_parsing_rules( ! $this->options->parser_dont_break_puncts,
				! $this->options->parser_dont_break_numbers, ! $this->options->parser_dont_break_entities );
...
//        $GLOBALS['wp_rewrite']->flush_rules();
	...
	public function on_parse_request( $wp ): void {
		...
		if ( ( $this->options->enable_detect_redirect || $this->options->widget_allow_set_deflang || $this->options->enable_geoip_redirect ) &&
		     ! ( $this->is_special_page( $_SERVER['REQUEST_URI'] ) || ( isset( $_SERVER['HTTP_REFERER'] ) && strpos( $_SERVER['HTTP_REFERER'],
					     $this->home_url ) !== false ) ) &&
			...
			if ( ! isset( $_SESSION['TR_REDIRECTED'] ) && ! ( isset( $_SERVER['HTTP_REFERER'] ) && $_SERVER['HTTP_REFERER'] == $_SERVER['REQUEST_URI'] ) ) {
				...
				if ( isset( $_COOKIE['TR_LNG'] ) && $this->options->widget_allow_set_deflang ) {
					if ( $_COOKIE['TR_LNG'] != $this->target_language ) {
						$url = transposh_utils::rewrite_url_lang_param( $_SERVER["REQUEST_URI"], $this->home_url,
							$this->enable_permalinks_rewrite, $_COOKIE['TR_LNG'], $this->edit_mode );
					...
					if ( $bestlang && $bestlang != $this->target_language ) {
						$url = transposh_utils::rewrite_url_lang_param( $_SERVER['REQUEST_URI'], $this->home_url,
							$this->enable_permalinks_rewrite, $bestlang, $this->edit_mode );
		...
		if ( isset( $wp->query_vars['s'] ) ) {
			...
			if ( transposh_utils::get_language_from_url( $_SERVER['HTTP_REFERER'],
					$this->home_url ) && ! transposh_utils::get_language_from_url( $_SERVER['REQUEST_URI'],
					$this->home_url ) ) {
				$this->tp_redirect( transposh_utils::rewrite_url_lang_param( $_SERVER["REQUEST_URI"], $this->home_url,
					$this->enable_permalinks_rewrite,
					transposh_utils::get_language_from_url( $_SERVER['HTTP_REFERER'], $this->home_url ),
					false ) ); //."&stop=y");
				exit;
			}
		}
		if ( isset( $wp->query_vars[ EDIT_PARAM ] ) && $wp->query_vars[ EDIT_PARAM ] && $this->is_editing_permitted() ) {
			...
			if ( transposh_utils::is_bot() ) {
				$this->tp_redirect( transposh_utils::rewrite_url_lang_param( $_SERVER["REQUEST_URI"], $this->home_url,
					$this->enable_permalinks_rewrite,
					transposh_utils::get_language_from_url( $_SERVER["REQUEST_URI"], $this->home_url ), false ), 301 );
				exit;
			}
		} else {
	 ...
	 */
	public function plugin_install_error(): void {
	 ...
	 */
	public function add_transposh_css(): void {
		...
		wp_enqueue_style( 'transposh', $this->transposh_plugin_url . '/' . TRANSPOSH_DIR_CSS . '/transposh.css',
			array(), TRANSPOSH_PLUGIN_VER );
...
//            $script_params['engines'][] = 'b';
		...
		if ( $this->options->oht_id && $this->options->oht_key && in_array( $this->target_language,
				transposh_consts::$oht_languages ) && current_user_can( 'manage_options' ) ) {
...
//          'l10n_print_after' => 'try{convertEntities(inlineEditL10n);}catch(e){};'
	 ...
	 */
	public function add_rel_alternate(): void {
		if ( is_404() ) {
			return;
		}
	 ...
	 */
	// TODO????
	public function is_editing_permitted(): bool {
	 ...
	 * TODO: move to options
	 */
	public function is_auto_translate_permitted(): bool {
	...
	public function rewrite_url( string $href ): string {
		...
		// first cut home
		if ( $this->options->enable_url_translate ) {
			$href = transposh_utils::translate_url( $href, $this->home_url, $this->target_language,
				array( &$this->database, 'fetch_translation' ) );
		}
		$href = transposh_utils::rewrite_url_lang_param( $href, $this->home_url, $this->enable_permalinks_rewrite,
			$this->target_language, $this->edit_mode, $use_params );
	...
	public function posts_where_request( $where ): string {
		...
		if ( ! empty( $q['s'] ) ) {
			...
			foreach ( (array) $q['search_terms'] as $term ) {
				...
				$possible_original_terms = $this->database->get_orignal_phrases_for_search_term( $term,
	...
	public function comment_text_wrap( $text ): string {
		...
		if ( $comment_lang ) {
			...
			if ( strpos( $text, '<a href="' . $this->home_url ) !== false ) {
				$text = str_replace( '<a href="' . $this->home_url,
					'<a lang="' . $this->options->default_language . '" href="' . $this->home_url, $text );
			}
		}
	...
	public function request_filter( $query ): array {
		...
		if ( $lang && ! $this->got_request ) {
			...
			$_SERVER['REQUEST_URI'] = transposh_utils::get_original_url( $requri, '', $lang,
				array( $this->database, 'fetch_original' ) );
			global $wp;
			$wp->parse_request();
			$query                  = $wp->query_vars;
...
//            }
	...
	// Super Proxy
	public function on_ajax_nopriv_proxy(): void {
		// Check if enabled
	...
	public function on_ajax_nopriv_tp_tp(): void {
		// we need curl for this proxy
		if ( ! function_exists( 'curl_init' ) ) {
			return;
		}
...
//
	...
	public function get_yandex_translation( $tl, $sl, $q ): bool {
		$sid       = '';
		$timestamp = 0;
	...
	public function hq( $a, $chunk ): int {
	 ...
	 */
	public function iq( $input, $error ): string {
		$e     = explode( ".", $error );
		$value = intval( $e[0] );
...
// Proxied translation for google translate
	 ...
	 */
	public function on_ajax_nopriv_tp_oht(): void {
		// Admin access only
	...
	public function on_ajax_nopriv_tp_history(): void {
		// deleting
		transposh_utils::allow_cors();
		if ( isset( $_POST['timestamp'] ) ) {
			$result = $this->database->del_translation_history( stripslashes( $_POST['token'] ), $_POST['lang'],
				$_POST['timestamp'] );
	...
	public function on_ajax_nopriv_tp_translation(): void {
		transposh_utils::allow_cors();
	 ...
	 */
	public function on_ajax_nopriv_tp_ohtcallback(): void {
		$ohtp = get_option( TRANSPOSH_OPTIONS_OHT_PROJECTS, array() );
		tp_logger( $ohtp );
	...
	public function on_ajax_nopriv_tp_trans_alts(): void {
		transposh_utils::allow_cors();
	...
	public function on_ajax_nopriv_tp_cookie(): void {
	...
	public function check_for_plugin_update( $checked_data ) {
		...
		$args           = array(
			...
			'version' => '1.0.7', //$checked_data->checked[$this->transposh_file_location],
		);
		...
		if ( is_wp_error( $request ) ) {
			$res = new WP_Error( 'plugins_api_failed',
				__( 'An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>',
					TRANSPOSH_TEXT_DOMAIN ), $request->get_error_message() );
		} else {
			$res = unserialize( $request['body'] );

			if ( $res === false ) {
				$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', TRANSPOSH_TEXT_DOMAIN ),
					$request['body'] );
			}
		}
