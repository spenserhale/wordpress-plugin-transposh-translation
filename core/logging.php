		...
		if ( $severity <= $this->debug_level ) {
			if ( $this->show_caller ) {
				...
				if ( isset( $trace[ 2 + $nest ]['class'] ) ) {
					$log_prefix = str_pad( "{$trace[2 + $nest]['class']}::{$trace[2 + $nest]['function']} {$trace[1 + $nest]['line']}",
						55 + $nest, '_' );
				} else {
					$prefile    = substr( $trace[ 1 + $nest ]['file'], strrpos( $trace[ 1 + $nest ]['file'], "/" ) );
					$log_prefix = str_pad( "{$prefile}::{$trace[1 + $nest]['function']} {$trace[1 + $nest]['line']}",
						55 + $nest, '_' );
				}
			}
			...
			if ( $this->printout/* || !isset($this->firephp)*/ ) {
				...
				if ( ! isset( $_SERVER['REMOTE_ADDR'] ) || $this->remoteip != $_SERVER['REMOTE_ADDR'] ) {
					return;
				}
