...
class tp_parser {
	...
	public function is_sentence_breaker( char $char, char $nextchar, $nextnextchar ): int {
		...
		//    return 2;
		return ( strpos( ',?()[]{}"!:|;' . TP_GTXT_BRK . TP_GTXT_BRK_CLOSER . TP_GTXT_IBRK . TP_GTXT_IBRK_CLOSER,
				$char ) !== false ) ? 1 : 0; // TODO: might need to add < and > here
	}
...
//                tp_logger("   $closers INNER text breaker $logstr start:$start pos:$pos gt:" . $this->in_get_text_inner, 3);
			...
			elseif ( $this->punct_breaks && $senb_len = $this->is_sentence_breaker( $string[ $pos ],
					@$string[ $pos + 1 ], @$string[ $pos + 2 ] ) ) {
...
//                logger ("numnum... $num_len");
				...
				if ( ( $start == $pos ) || ( $this->is_white_space( $string[ $pos - 1 ] ) || ( $this->is_sentence_breaker( @$string[ $pos + $num_len - 1 ],
							@$string[ $pos + $num_len ], @$string[ $pos + $num_len + 1 ] ) ) ) &&
				                           ( $this->is_white_space( @$string[ $pos + $num_len ] ) || $this->is_sentence_breaker( @$string[ $pos + $num_len ],
						                           @$string[ $pos + $num_len + 1 ],
						                           @$string[ $pos + $num_len + 2 ] ) ) ) {
...
//                            logger ("compensate part2?");
						if ( $this->is_sentence_breaker( $string[ $pos + $num_len - 1 ], @$string[ $pos + $num_len ],
							@$string[ $pos + $num_len + 1 ] ) ) {
//                            logger ("compensate 3?");
...
//                logger ("numnumpos... $pos");
	...
	public function translate_tagging( simple_html_dom_node $node, $level = 0 ): void {
		...
		if ( stripos( $node->class, NO_TRANSLATE_CLASS ) !== false || stripos( $node->class,
				NO_TRANSLATE_CLASS_GOOGLE ) !== false ) {
			return;
		}
		...
		if ( isset( $src_set_here ) && $src_set_here && $this->srclang != $this->lang && stripos( $node->class,
				ONLY_THISLANGUAGE_CLASS ) !== false ) {
...
//        $originals = array();
			...
			foreach ( $e->nodes as $ep ) {
				...
				if ( ( $this->is_edit_mode || ( $this->is_auto_translate && $translated_text == null ) )/* && $ep->inbody */ ) {
					if ( $ep->inselect ) {
						...
						$hiddenspans .= $this->create_edit_span( $ep->phrase, $translated_text, $source, true,
							$ep->srclang );
					} else {
						$translated_text = $this->create_edit_span( $ep->phrase, $translated_text, $source, false,
							$ep->srclang );
					}
				}
				// store replacements
		...
		foreach ( array( 'title', 'value', 'placeholder', 'alt' ) as $title ) {
			foreach ( $this->html->find( '[' . $title . ']' ) as $e ) {
				...
				if ( isset( $e->parent->_[ HDOM_INFO_OUTER ] ) ) {
					$e->parent->outertext = implode( $e->outertext,
						explode( $saved_outertext, $e->parent->outertext, 2 ) );
				}
			}
		}
