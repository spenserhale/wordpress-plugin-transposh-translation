...
//        $gluebackhome = true;
	...
	public static function prefered_language(
		...
		preg_match_all( "/([[:alpha:]]{1,8})(-([[:alpha:]|-]{1,8}))?" .
		                "(\s*;\s*q\s*=\s*(1\.0{0,3}|0\.\d{0,3}))?\s*(,|$)/i", $http_accept_language, $hits,
			PREG_SET_ORDER );
		...
		foreach ( $hits as $arr ) {
			...
			elseif ( in_array( $langprefix, $available_languages ) && ( ( $qvalue * 0.9 ) > $bestqval ) ) { // CHECK!
				$bestlang = $langprefix;
				$bestqval = $qvalue * 0.9;
			}
		}
	...
	public static function is_bot() {
		return preg_match( "#(bot|yandex|validator|google|jeeves|spider|crawler|slurp)#si",
